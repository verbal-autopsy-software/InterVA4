...
#' 
InterVA<-function(Input,HIV,Malaria,directory = NULL, filename = "VA_result", output="classic", append=FALSE, groupcode = FALSE, replicate = FALSE, replicate.bug1 = FALSE, replicate.bug2 = FALSE, write = TRUE, external.sep = TRUE){

    removeExt <- function(data,prob.orig, is.Numeric, subpop){
            extSymps <- seq(211, 222)
            extCauses <- seq(41, 51)
            ...
            data <- data[-ext.where, ]
            # delete all external symptoms, mark as missing, so it'll be deleted after
            # if(is.Numeric){ ext.sub
            #   data[, extSymps + 1] <- -1
            # }else{
            #   data[, extSymps + 1] <- "."
            # }
...
save.va.prob <- function(x, filename, write){
    ...
    # separate externals
    if(external.sep){
        externals <- removeExt(Input, matrix(as.numeric(probbase[2:246, 17:76]), nrow = 245), FALSE, NULL)
        Input <- externals$data
        extSymps <- seq(211, 222)
        extCauses <- seq(41, 51) 
        probbase <- probbase[-(extSymps+1), -(extCauses + 16)]
        N <- dim(Input)[1]
        S <- dim(Input)[2]
        C <- 60 - length(extCauses)        
    }
    ######################################################################
    ...
    for(i in 1:N){
        ...
        # Normalize B group 
		if(sum(prob[4:(C+3)]) > 0) prob[4:(C+3)] <- prob[4:(C+3)]/sum(prob[4:(C+3)])
        # delete too small probs
        if(replicate.bug2){prob[prob < 0.000001] <- 0}
        }
              
        if(external.sep){
            names(prob) <- causetext[-(extCauses+3),2]
        }else{
            names(prob) <- causetext[,2]
        }

        prob_A <- prob[1:3] # Extracting only A_group
        prob_B <- prob[4:(C+3)] # Extracting only COD
    ...
    if(external.sep){
                temp <- output[, 41:49]
                extra <- matrix(0, length(externals$ext.id), 60)
                for(id in 1:length(externals$ext.id)){
                    extra[id, externals$ext.cod[id]] <- 1
                }
                output <- cbind(output[, 1:40], matrix(0, dim(output)[1], 11), temp)
                output <- rbind(output, extra)
    }
    if(!is.null(toremove)) output <- output[-toremove, ]
    # setwd(globle.dir)
